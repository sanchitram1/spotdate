...
def extract(listening_history: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature in ['valence', 'energy', 'danceability', 'popularity', 'acousticness',
    ...
    df['explicit_mode'] = df.groupby('user_id')['explicit'].transform(lambda x: x.mode()[0])
    ...
    for feature in ['valence', 'energy', 'tempo', 'danceability', 'loudness']:
        ...
        night_mean = night.groupby('user_id')[feature].mean().rename(f'night_{feature}_mean')
    ...
    df['pct_zero_popularity']    = df.groupby('user_id')['popularity'].transform(lambda x: (x == 0).mean())
    df['pct_viral_tracks']       = df.groupby('user_id')['popularity'].transform(lambda x: (x > 80).mean())
    df['pct_underground_tracks'] = df.groupby('user_id')['popularity'].transform(lambda x: (x < 20).mean())
    df['pct_explicit']           = df.groupby('user_id')['explicit'].transform('mean')
    df['pct_fast_tracks']        = df.groupby('user_id')['tempo'].transform(lambda x: (x > 150).mean())
    df['pct_slow_tracks']        = df.groupby('user_id')['tempo'].transform(lambda x: (x < 80).mean())
    df['pct_high_energy']        = df.groupby('user_id')['energy'].transform(lambda x: (x > 0.7).mean())
    df['pct_low_energy']         = df.groupby('user_id')['energy'].transform(lambda x: (x < 0.3).mean())
    df['pct_happy_tracks']       = df.groupby('user_id')['valence'].transform(lambda x: (x > 0.6).mean())
    df['pct_sad_tracks']         = df.groupby('user_id')['valence'].transform(lambda x: (x < 0.4).mean())
    df['pct_live']               = df.groupby('user_id')['liveness'].transform(lambda x: (x > 0.8).mean())
    df['pct_speech']             = df.groupby('user_id')['speechiness'].transform(lambda x: (x > 0.66).mean())
    ...
    cols_to_keep = [
        'user_id',
        'avg_valence', 'avg_energy', 'avg_danceability', 'avg_popularity',
        'avg_acousticness', 'avg_instrumentalness', 'avg_liveness',
        'avg_loudness', 'avg_speechiness', 'avg_tempo', 'avg_key',
        'explicit_mode',
        'variance_valence', 'variance_energy', 'variance_danceability',
        'variance_popularity', 'variance_acousticness', 'variance_instrumentalness',
        'variance_liveness', 'variance_loudness', 'variance_speechiness',
        'variance_tempo', 'variance_key',
        'emotional_state',
        'pct_zero_popularity', 'pct_viral_tracks', 'pct_underground_tracks',
        'pct_explicit', 'pct_fast_tracks', 'pct_slow_tracks',
        'pct_high_energy', 'pct_low_energy', 'pct_happy_tracks',
        'pct_sad_tracks', 'pct_live', 'pct_speech',
        'night_energy_mean', 'day_energy_mean', 'night_valence_mean', 'day_valence_mean',
        'night_tempo_mean', 'day_tempo_mean', 'night_danceability_mean', 'day_danceability_mean',
        'night_loudness_mean', 'day_loudness_mean',
        'valence_evolution', 'energy_evolution', 'tempo_evolution',
        'danceability_evolution', 'acousticness_evolution'
    ]
