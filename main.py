...
@app.get("/api/data/audio-features")
async def get_audio_features(user_id: str, time_range: str = "medium_term"):
    """Get audio feature analysis of user's top tracks"""
    if user_id not in user_tokens:
        raise HTTPException(status_code=401, detail="Not authenticated")
...
@app.get("/api/data/export-track-features")
async def export_track_features(user_id: str, time_range: str = "short_term"):
    """Get audio features for the first top track"""
    if user_id not in user_tokens:
        raise HTTPException(status_code=401, detail="Not authenticated")
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            # 2) Get audio features for this one track
            features_response = await client.get(
            ...
            return {
                "track": {
                    ...
                    "artist": first_track.get("artists", [{}])[0].get("name")
                    if first_track.get("artists")
                    else "Unknown",
                    "id": track_id,
                    "popularity": first_track.get("popularity"),
                },
                "audio_features": {
