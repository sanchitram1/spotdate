...
class QuirksIdea(ImplementationIdea):
    ...
    def build(self, context: PairContext, config: DashboardConfig = CONFIG) -> QuirksPayload:
        green_flags = []
        selected_quirks = []
        match_quirks = []
        ...
        return QuirksPayload(
            ...
            selected_quirks=selected_quirks if selected_quirks else ["Pretty well-rounded listener"],
            match_quirks=match_quirks if match_quirks else ["Pretty well-rounded listener"]
        )

    def render(self, payload: QuirksPayload, context: PairContext, config: DashboardConfig = CONFIG) -> None:
        st.markdown(
            f"""
