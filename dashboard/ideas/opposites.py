...
class OppositesIdea(ImplementationIdea):
    ...
    def build(self, context: PairContext, config: DashboardConfig = CONFIG) -> OppositesPayload:
        ...
        top_diffs = df.sort_values("diff_magnitude", ascending=False).head(3).reset_index(drop=True)
        return OppositesPayload(differences=top_diffs)

    def render(self, payload: OppositesPayload, context: PairContext, config: DashboardConfig = CONFIG) -> None:
        st.markdown(
            f"""
        ...
        for i in range(len(y_labels)):
            figure.add_shape(
                type="line",
                x0=selected_vals[i], y0=y_labels[i],
                x1=match_vals[i], y1=y_labels[i],
                line=dict(color=config.style.text_muted, width=3, dash="dot"),
                layer="below"
            )

        figure.update_layout(
            xaxis=dict(range=[0, 1], tickformat=".0%", title="Percentile Score", gridcolor="rgba(255,255,255,0.1)"),
